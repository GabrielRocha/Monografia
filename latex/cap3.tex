...
write list = gabriel # Somente o usuario gabriel tera permissao de escrita no compartilhamento.

read list = felipe # O usuario felipe so podera ler e listas os arquivos e diretorios desse compartilhamento.
...
Exemplo de um script diretoria.bat no Quadro \ref{net_use}.\\
...
O resultado da execução do \textit{script} após o \textit{login} pode ser visto na Figura \ref{mapeamento}
...
\begin{enumerate}
	...
	\begin{figure}[ht]
   			\centering
   			\includegraphics[width=0.5 \textwidth]{figuras/logon_local_adm}
...
\pagebreak 
	...
	\item {Acessar a aba ``Nome do Computador". Deve-se clicar no botão ``Alterar". - Figura \ref{alterar_nome_micro}}
		...
		\begin{figure}[ht]
	   			\centering
	   		 	\includegraphics[width=0.4 \textwidth]{figuras/alterar_nome_micro}
	   			\caption{Alterando nome do micro}
	    		\label{alterar_nome_micro}
		\end{figure}		 

	\item {No menu de ``Alterações de nome do computador", informe o nome do computador que foi cadastrado no servidor Samba 3. No campo ``Membro de", selecione a opção ``Domínio" e digite o nome do definido no \textit{WORKGROUP} no Samba 3 e depois clique em OK. - Figura \ref{incluir_dominio}} 
			\begin{figure}[ht]
		   			\centering
		   		 	\includegraphics[width=0.4 \textwidth]{figuras/incluir_dominio}
...
\pagebreak
	...
	\begin{figure}[ht]
			\centering
	 		\includegraphics[width=0.6 \textwidth]{figuras/logon}
...
\begin{enumerate}
	...
	\item \textbf{\# mkdir x64 W32X86} - Essas pastas são os locais onde ficarão os drivers das impressoras, o x64 para sistemas 64 bits e o W32X86 Windows NT/2000/XP.
	\item \textbf{\# chown dtic x64 W32X86} - Muda o proprietário da pasta para que o usuário dtic tenha permissão para escrever no diretório.
...
\begin{enumerate}
	\item \textbf{Acesse com o usuário com permissão para gerenciar as impressoras} - Figura \ref{login_windows_local}
		\begin{figure}[ht]
		   	\centering
		    \includegraphics[width=0.5 \textwidth]{figuras/login_dominio_dtic}
	...
	\begin{figure}[ht]
	   	\centering
	     \includegraphics[width=0.6 \textwidth]{figuras/selecionar_so}
	...
	\begin{figure}[ht]
	   	\centering
	     \includegraphics[width=0.5 \textwidth]{figuras/propriedade_impressora}
...
\pagebreak
	...
	\begin{figure}[ht]
	   	\centering
	     \includegraphics[width=1 \textwidth]{figuras/opcao_nao}
